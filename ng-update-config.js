 ...
 * 1. Place this gist file `ng-update-config.js` to angular project's root path
 * 2. use command `node ng-update-config` 
 * 3. check angular.json file (created by ng-update-config.js)
 **/
const fs = require('fs');
...
const legacyJSONPath = path.resolve(__dirname, '.angular-cli.json');
...
fs.writeFileSync("angular.json", JSON.stringify(newJSON, null, 4));
console.log(newJSON);