...
const baseProp = (rawOptions, Type, target, key, isArray = false) => {
    ...
    if (!subSchema && !(utils_1.isPrimitive(Type) || utils_1.isMongoose(Type) || utils_1.isAny(Type))) {
        throw new errors_1.InvalidPropError(Type.name, key);
    }
    const options = _.omit(rawOptions, ['ref', 'items']);
    if (utils_1.isPrimitive(Type) || utils_1.isMongoose(Type)) {
        if (isArray) {
    ...
    if (utils_1.isAny(Type)) {
        if (isArray) {
            data_1.schema[name][key][0] = Object.assign({}, data_1.schema[name][key][0], options, { type: mongoose.Schema.Types.Mixed });
            return;
        }
        data_1.schema[name][key] = Object.assign({}, data_1.schema[name][key], options, { type: mongoose.Schema.Types.Mixed });
        return;
    }
    if (isArray) {
